        ...
        // Toàn bộ JS đã có sẵn trong file này (từ Qlsanpham.js) – nhưng tôi để nguyên bên dưới
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const hinAnhOld = document.getElementById('hinAnhOld');
            ...
            function renderProducts(products) {
                if (!products || products.length === 0) {
                    productTableBody.innerHTML = '<tr><td colspan="8" style="text-align:center;">Không có sản phẩm nào</td></tr>';
                    return;
                }
                let html = '';
                products.forEach(p => {
                    // --- VỊ TRÍ THAY ĐỔI ĐOẠN ĐỊNH NGHĨA ẢNH ---
                    // --- THAY THẾ ĐOẠN ĐỊNH NGHĨA imgHtml TRONG FUNCTION renderProducts ---
                const textAnh = p.HinAnh ? p.HinAnh.trim() : '';
                // Nếu có tên file ảnh thì nối với thư mục uploads/, nếu không có thì dùng ảnh mặc định luôn
                const srcAnh = textAnh ? 'uploads/' + textAnh : 'https://placehold.co/50x50?text=No+Image';
                ...
                // ---------------------------------------------------------------------
                    ...
                    html += `
                        <tr>
                            ...
                            <td>${p.GiaBan}</td>
                            <td>${imgHtml}</td> <td>${p.NgayCapNhat ? new Date(p.NgayCapNhat).toLocaleDateString('vi-VN') : ''}</td>
                            <td>${p.HanSuDung || ''}</td>
                ...
                if (maSanPham) {
                    ...
                    fetchAPI('get_product', { id: maSanPham }).then(data => {
                        if (data) {
                            ...
                            hinAnhOld.value = data.HinAnh || '';
                            if (data.HinAnh) {
                                currentImage.innerHTML = 'Ảnh hiện tại: <img src="uploads/' + data.HinAnh + '">';
                            } else {
            ...
            function saveProduct() {
                ...
                if (productImage.files[0]) {
                    formData.append('HinAnh', productImage.files[0]);
                }
                if (isEdit) {
                    formData.append('HinAnhOld', hinAnhOld.value);
                }
            ...
            function loadCategories() {
                fetchAPI('get_categories').then(data => {
                    if (data.categories) {
